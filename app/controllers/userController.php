<?php
    ...
    class userController extends mainModel{
        ...
        public function registrarUsuarioControlador(){
            ...
            # Verificando campos obligatorios (ahora incluye Cargo) #
            if($nombre=="" || $apellido=="" || $usuario=="" || $clave1=="" || $clave2=="" || $cargo==""){
                $alerta=[
                    "tipo"=>"simple",
                    "titulo"=>"Ocurrió un error inesperado",
                    "texto"=>"No has llenado todos los campos que son obligatorios (incluyendo el Cargo)",
                    "icono"=>"error"
                ];
            ...
            /* --- Verificando integridad de los nuevos datos --- */

            // Verificando Cédula (Ejemplo de RegEx para números, campo opcional)
            if($cedula!="" && $this->verificarDatos("[0-9]{5,20}",$cedula)){
                $alerta=[
                    "tipo"=>"simple",
                    "titulo"=>"Ocurrió un error inesperado",
                    "texto"=>"El formato de la CÉDULA/ID no es válido",
                    "icono"=>"error"
                ];
                return json_encode($alerta);
            }
            
            // Verificando Teléfono (RegEx para números y caracteres comunes de teléfono, campo opcional)
            if($telefono!="" && $this->verificarDatos("[0-9()+ -]{7,20}",$telefono)){
                $alerta=[
                    "tipo"=>"simple",
                    "titulo"=>"Ocurrió un error inesperado",
                    "texto"=>"El formato del NÚMERO DE TELÉFONO no es válido",
                    "icono"=>"error"
                ];
                return json_encode($alerta);
            }

            // Verificando Dirección (solo longitud, campo opcional)
            if($direccion!="" && strlen($direccion)>70){
                 $alerta=[
                    "tipo"=>"simple",
                    "titulo"=>"Ocurrió un error inesperado",
                    "texto"=>"La DIRECCIÓN no debe exceder los 70 caracteres",
                    "icono"=>"error"
                ];
                return json_encode($alerta);
            }
            
            // Verificando Cargo (solo longitud, campo obligatorio ya verificado)
            if(strlen($cargo)>30){
                 $alerta=[
                    "tipo"=>"simple",
                    "titulo"=>"Ocurrió un error inesperado",
                    "texto"=>"El Cargo no debe exceder los 30 caracteres",
                    "icono"=>"error"
                ];
                return json_encode($alerta);
            }

            /* --- Fin de verificación de nuevos datos --- */
            ...
            $usuario_datos_reg=[
                ...
                /* --- AÑADIENDO NUEVOS CAMPOS PARA INSERCIÓN --- */
                [
        ...
        public function listarUsuarioControlador($pagina,$registros,$url,$busqueda){
            ...
            if(isset($busqueda) && $busqueda!=""){
                ...
                // NOTA: Se recomienda añadir busqueda por los nuevos campos (cedula, telefono, cargo) para hacerla más útil
                $consulta_datos="SELECT * FROM usuario WHERE ((usuario_id!='".$_SESSION['id']."' AND usuario_id!='1') AND (usuario_nombre LIKE '%$busqueda%' OR usuario_apellido LIKE '%$busqueda%' OR usuario_email LIKE '%$busqueda%' OR usuario_usuario LIKE '%$busqueda%' OR usuario_cedula LIKE '%$busqueda%' OR usuario_cargo LIKE '%$busqueda%' OR usuario_telefono LIKE '%$busqueda%')) ORDER BY usuario_nombre ASC LIMIT $inicio,$registros";

                $consulta_total="SELECT COUNT(usuario_id) FROM usuario WHERE ((usuario_id!='".$_SESSION['id']."' AND usuario_id!='1') AND (usuario_nombre LIKE '%$busqueda%' OR usuario_apellido LIKE '%$busqueda%' OR usuario_email LIKE '%$busqueda%' OR usuario_usuario LIKE '%$busqueda%' OR usuario_cedula LIKE '%$busqueda%' OR usuario_cargo LIKE '%$busqueda%' OR usuario_telefono LIKE '%$busqueda%'))";
                ...
                <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                    <thead>
                        <tr>
                            ...
                            <th class="has-text-centered">Cédula</th>
            ...
            if($total>=1 && $pagina<=$numeroPaginas){
                ...
                if($total>=1){
                    // Si la paginación es incorrecta, recargar
                    $tabla.='
                        <tr class="has-text-centered" >
                            <td colspan="10">
        ...
        public function actualizarUsuarioControlador(){
            ...
            // ... (Bloque de verificación del Administrador, sin cambios) ...
            ...
            /* --- NUEVOS CAMPOS AGREGADOS para Actualización --- */
            $direccion=$this->limpiarCadena($_POST['usuario_direccion']);
            $telefono=$this->limpiarCadena($_POST['usuario_telefono']);
            $cedula=$this->limpiarCadena($_POST['usuario_cedula']);
            $cargo=$this->limpiarCadena($_POST['usuario_cargo']);
            ...
            # Verificando campos obligatorios (ahora incluye Cargo) #
            if($nombre=="" || $apellido=="" || $usuario=="" || $cargo==""){
                $alerta=[
                    "tipo"=>"simple",
                    "titulo"=>"Ocurrió un error inesperado",
                    "texto"=>"No has llenado todos los campos que son obligatorios (incluyendo el Cargo)",
            ...
            // ... (Bloque de verificación de formato de datos, sin cambios) ...
            ...
            /* INICIO DE MODIFICACIÓN: Verificando unicidad de Cédula (UNIQUE en DB) */
            if($cedula!="" && $datos['usuario_cedula']!=$cedula){
                $check_cedula=$this->ejecutarConsulta("SELECT usuario_cedula FROM usuario WHERE usuario_cedula='$cedula'");
                if($check_cedula->rowCount()>0){
                    $alerta=[
                        "tipo"=>"simple",
                        "titulo"=>"Ocurrió un error inesperado",
                        "texto"=>"La CÉDULA ingresada ya se encuentra registrada por otro usuario, por favor verifique e intente nuevamente",
                        "icono"=>"error"
                    ];
                    return json_encode($alerta);
                }
            ...
            $usuario_datos_up=[
                ...
                /* --- AÑADIENDO NUEVOS CAMPOS PARA ACTUALIZACIÓN --- */
                [
                    "campo_nombre"=>"usuario_direccion",
                    "campo_marcador"=>":Direccion",
                    "campo_valor"=>$direccion
        ...
        public function listarSecreariosControlador($pagina,$registros,$url,$busqueda){
            ...
            if($total>=1 && $pagina<=$numeroPaginas){
                ...
                if($rows['usuario_cargo'] === "Secretario"){
                    $tabla.='
                            <tr class="has-text-centered" >
                                <td>'.$contador.'</td>
                                <td>'.$rows['usuario_nombre'].' '.$rows['usuario_apellido'].'</td>
